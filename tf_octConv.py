...
def firstOctConv(data, settings, ch_in, ch_out, name, kernel=(1,1), pad='valid', stride=(1,1)):
    ...
    if stride == (2, 2):
        hf_data = Pooling(data=hf_data, pool_type='avg', kernel=(2,2), stride=(2,2), name=None)
    hf_conv = Conv(data=hf_data, num_filter=hf_ch_out, kernel=kernel, pad=pad, stride=(1,1), name=None)
    hf_pool = Pooling(data=hf_data, pool_type='avg', kernel=(2,2), stride=(2,2), name=None)
    hf_pool_conv = Conv(data=hf_pool, num_filter=lf_ch_out, kernel=kernel, pad=pad, stride=(1,1), name=None)
...
def lastOctConv(hf_data, lf_data, settings, ch_in, ch_out, name, kernel=(1,1), pad='valid', stride=(1,1)):
    ...
    lf_conv = Conv(data=lf_data, num_filter=hf_ch_out, kernel=kernel, pad=pad, stride=(1,1), name=None)
    if stride == (1, 1):
        lf_conv = UpSampling(lf_conv, scale=2, sample_type='nearest',num_args=1, name=None)
...
def OctConv(hf_data, lf_data, settings, ch_in, ch_out, name, kernel=(1,1), pad='valid', stride=(1,1)):
    ...
    if stride == (2, 2):
        lf_upsample = lf_conv
        lf_down = Pooling(data=lf_data, pool_type='avg', kernel=(2,2), stride=(2,2), name=None)
    else:
        lf_upsample = UpSampling(lf_conv, scale=2, sample_type='nearest',num_args=1, name=None)
        lf_down = lf_data
    lf_down_conv = Conv(data=lf_down, num_filter=lf_ch_out, kernel=kernel, pad=pad, stride=(1,1), name=None)
...
def firstOctConv_BN_AC(data, alpha, num_filter_in, num_filter_out,  kernel, pad, stride=(1,1), name=None, w=None, b=None, no_bias=True, attr=None, num_group=1):
    hf_data, lf_data = firstOctConv(data=data, settings=(0, alpha), ch_in=num_filter_in, ch_out=num_filter_out, name=None, kernel=kernel, pad=pad, stride=stride)
    out_hf = BN_AC(data=hf_data, name=None)
    out_lf = BN_AC(data=lf_data, name=None)
...
def lastOctConv_BN_AC(hf_data, lf_data, alpha, num_filter_in, num_filter_out,  kernel, pad, stride=(1,1), name=None, w=None, b=None, no_bias=True, attr=None, num_group=1):
    conv = lastOctConv(hf_data=hf_data, lf_data=lf_data, settings=(alpha, 0), ch_in=num_filter_in, ch_out=num_filter_out, name=None, kernel=kernel, pad=pad, stride=stride)
    out = BN_AC(data=conv, name=None)
    return out
...
def octConv_BN_AC(hf_data, lf_data, alpha, num_filter_in, num_filter_out,  kernel, pad, stride=(1,1), name=None, w=None, b=None, no_bias=True, attr=None, num_group=1):
    hf_data, lf_data = OctConv(hf_data=hf_data, lf_data=lf_data, settings=(alpha, alpha), ch_in=num_filter_in, ch_out=num_filter_out, name=None, kernel=kernel, pad=pad, stride=stride)
...
def firstOctConv_BN(data, alpha, num_filter_in, num_filter_out,  kernel, pad, stride=(1,1), name=None, w=None, b=None, no_bias=True, attr=None, num_group=1):
    hf_data, lf_data = firstOctConv(data=data, settings=(0, alpha), ch_in=num_filter_in, ch_out=num_filter_out, name=None, kernel=kernel, pad=pad, stride=stride)
    out_hf = BN(data=hf_data, name=None)
    out_lf = BN(data=lf_data, name=None)
    return out_hf, out_lf
